...
async def submit_contact_form(
    ...
    new_contact = await contact_service.create_contact(contact)
...
async def list_contacts(
    ...
    contact_service = ContactService()
    return await contact_service.list_contacts(skip=skip, limit=limit, unread_only=unread_only)
...
async def mark_contact_read(
    ...
    contact_service = ContactService()
    return await contact_service.mark_as_read(contact_id)
...
async def send_message_to_customers(
    ...
    customers = []
    for customer_id in message_data.recipient_ids:
        try:
            customer = await contact_service.get_contact(customer_id)
    ...
    if message_data.send_via_sms:
        ...
        phone_list = [c.phone for c in customers if c.phone]
